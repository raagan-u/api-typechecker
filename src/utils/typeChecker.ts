import { ValidationSchema, ExpectedType } from '../types/validationSchema';

// Helper function to validate the primitive types
...
export function typeCheck(request: any, schema: ValidationSchema): boolean {
  for (const key in schema) {
    ...
    // If expectedType is an object, recurse for nested validation
      ...
      if (!typeCheck(request[key], expectedType as ValidationSchema)) {
        return false;
      }
    }
  }
